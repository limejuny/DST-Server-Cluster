...
AddClassPostConstruct("widgets/hoverer", function(self)
	...
	function self:OnUpdate()
		...
		local str = nil
		if self.owner then
			if self.isFE == false and self.owner.HUD then
		...
		if not str and self.isFE == false then
			local LeftMouseB = nil
			if self.owner and self.owner.components then
				LeftMouseB = self.owner.components.playercontroller:GetLeftMouseAction()
			end
			if LeftMouseB then
